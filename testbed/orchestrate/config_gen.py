"""Generate strongSwan configuration from a parameter set.

Step 1.3 wrote two configs by hand to prove the mechanism. This module turns that
into a parameter space the Step 1.5 matrix can sweep, and it is where the protocol's
consistency rules are enforced: AEAD ciphers carry their own integrity and must not be
paired with a separate algorithm, non-AEAD ciphers cannot go without one, and IKEv2 has
no aggressive mode.

Rendered configurations contain **no secret**. The pre-shared key is supplied at run
time through the environment and written to ``conf.d/psk.conf`` by the container
entrypoint, so no credential is ever written to disk in this repository.
"""
