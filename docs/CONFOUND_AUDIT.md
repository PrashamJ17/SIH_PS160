...
*Generated 2026-09-06 09:56 UTC by `scripts/confound_audit.py`.*
...
Overall traffic-classification accuracy: **96.4%** over
637 windows, grouped 5-fold by capture.
...
Spread between best and worst: **1.5%**
(tolerance 15%).
...
| Measure | Value |
|---|---|
| Cipher accuracy from sizes | **76.0%** |
| Majority-class baseline | 47.9% |
| Lift over baseline | **+28.1%** |

## 3. Mutual information with cipher

| Quantity | Nats | Share of traffic-class entropy |
|---|---|---|
| True labels vs cipher | 0.0018 | 0.1% |
| Predicted labels vs cipher | 0.0022 | 0.1% |

Traffic-class entropy: 1.9280 nats.

## Verdict

**Accuracy does not depend on the cipher.** The spread between the best and worst cipher is 1.5%, inside the 15% tolerance. A classifier reading a cipher artifact would score visibly better under the cipher whose artifact it had learned.

**The cipher is identifiable from ESP packet sizes alone at 76.0%, against a 47.9% majority-class baseline.** This is a real finding rather than a defect: AES-CBC pads to a 16-byte block boundary, 3DES to 8 bytes, and AES-GCM not at all, so ESP packet sizes carry the cipher's block structure. **A passive observer can therefore infer the negotiated cipher without ever seeing a handshake** — useful when a capture starts mid-session, and a reminder that padding is a side channel.
...
**The classifier's output carries no more cipher information than the true labels do.** Mutual information with cipher is 0.0022 nats for the predictions against 0.0018 for the ground truth (0.1% and 0.1% of traffic-class entropy). The model is not manufacturing a cipher dependency the corpus does not have.
